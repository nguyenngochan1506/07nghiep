...
import { CreditCard, ReceiptText, Save, TicketPercent } from "lucide-react";
  ...
  const createVoucherMutation = useMutation({
    mutationFn: () =>
      trpcClient.admin.billing.createVoucher.mutate({
        ...
        maxDiscountVnd: optionalNumber(voucherForm.maxDiscountVnd),
        startsAt: optionalDate(voucherForm.startsAt),
        expiresAt: optionalDate(voucherForm.expiresAt),
        usageLimit: optionalNumber(voucherForm.usageLimit),
        perUserLimit: optionalNumber(voucherForm.perUserLimit),
        planIds: voucherForm.planIds,
      }),
    onSuccess: () => {
          ...
          <div className="mx-auto w-full max-w-5xl rounded-xl border bg-surface-wash/70 p-4">
            <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <VoucherField
                  label={voucherForm.discountType === "PERCENT" ? "Phần trăm giảm" : "Số tiền giảm"}
                  hint={voucherForm.discountType === "PERCENT" ? "Nhập 100 để miễn phí." : undefined}
                >
                  <Input
                    inputMode="numeric"
                    ...
                    placeholder={voucherForm.discountType === "PERCENT" ? "VD: 100" : "VD: 50000"}
                    className="bg-background"
                  />
                </VoucherField>
                {voucherForm.discountType === "PERCENT" ? (
                  <VoucherField label="Trần giảm" hint="Bỏ trống nếu không giới hạn.">
                    <Input
                      inputMode="numeric"
                      value={voucherForm.maxDiscountVnd}
                      onChange={(event) =>
                        setVoucherForm((current) => ({
                          ...current,
              ...
              <div className="flex flex-col gap-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <VoucherField label="Bắt đầu">
                    <Input
                      ...
                      onChange={(event) =>
                        setVoucherForm((current) => ({ ...current, startsAt: event.target.value }))
                      }
                      className="bg-background"
                    />
                  </VoucherField>
                  <VoucherField label="Kết thúc">
                    <Input
                      ...
                      onChange={(event) =>
                        setVoucherForm((current) => ({ ...current, expiresAt: event.target.value }))
                      }
                      className="bg-background"
                    />
                  </VoucherField>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid gap-2">
                    {plans.map((plan) => {
                      const checked = voucherForm.planIds.includes(plan.id);
